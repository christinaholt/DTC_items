...
for CurDate in `cat $1`; do
   ...
   TarFile="/NCEPPROD/hpssprod/runhistory/rh${YYYY}/${YYYY}${MM}/${YYYY}${MM}${DD}/com2_ccpa_prod_ccpa.${YYYY}${MM}${DD}.tar"
   #TarCommand="htar -xvf ${TarFile} \`htar -tf ${TarFile} | egrep \"ccpa_conus_hrap\" | awk '{print $7}'\`" 
   TarCommand="htar -xvf ${TarFile} \`htar -tf ${TarFile} | egrep \"06h.hrap.conus.gb2\" | awk '{print $7}'\`" 
   echo "CALLING: time ${TarCommand}"
   time htar -xvf ${TarFile} `htar -tf ${TarFile} | egrep "06h.hrap.conus.gb2" | awk '{print $7}'`
   Status=$?
   cd ..
